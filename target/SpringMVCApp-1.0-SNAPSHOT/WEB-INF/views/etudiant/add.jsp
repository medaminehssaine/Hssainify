...
<div class="container">
    ...
    <h3 class="mb-3">
        <c:choose>
            <c:when test="${model.etudiant != null}">Edit Student</c:when>
            <c:otherwise>Add Student</c:otherwise>
        </c:choose>
    </h3>

    <!-- ✔ FORMULAIRE /etudiants/save -->
    <form method="post" action="${pageContext.request.contextPath}/etudiants/save">

        <!-- ✔ ID caché si édition -->
        <c:if test="${model.etudiant != null}">
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="filiere" required>
                ...
                <c:forEach var="f" items="${model.filieres}">
                    <option value="${f.id}"
                        <c:if test="${model.etudiant != null && f.id == model.etudiant.filiere.id}">
                            selected
                        </c:if>>
                        ${f.nom}
                    </option>
        ...
        <button class="btn btn-success">
            <c:choose>
                <c:when test="${model.etudiant != null}">Update</c:when>
